...
- 新增：主动说话功能（desire/active_send.py）。当很想对方、很开心、情绪低落或对方长时间没联系时，由 AI 主动发一条消息。内容优先走 LLM 按当前状态生成，失败回退模板。含冷却时间、每日上限、对方在场不打扰等规则。
- 修复：主动说话消息偶发"只有话头没有内容"的空壳（例如只发"我问你个事"却无下文）。提示词增加硬约束（话要一次说完、没有具体事就直接说想念、禁止编话头），并新增空话头检测，命中即回退真心话模板。
- 修复：内心独白区分"对方在场 / 不在场"两套模板（desire/monologue.py、desire/integration.py），对方在场时不再冒出"她再不来我就要自己找她了"的错位独白。
- 配置脱敏：LLM key、API 地址、目标 QQ 均改为环境变量读取（DESIRE_LLM_API_KEY / DESIRE_LLM_API_BASE / DESIRE_TARGET_QQ），仓库内不含任何私密凭据。
